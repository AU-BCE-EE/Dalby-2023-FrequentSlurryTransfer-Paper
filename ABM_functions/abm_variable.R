abm_variable <-
function(days, delta_t, times, y, pars, warn, temp_C_fun = temp_C_fun, temp_air_C_fun = temp_air_C_fun, pH_fun = pH_fun, H2SO4_inhibition_fun = H2SO4_inhibition_fun) {
  ...
  for (i in 2:n_int) {
    ...
    if (!removals[i]) {
      ...
      y <- unlist(out[nrow(out), 1:(length(c(pars$qhat_opt)) + 15) + 1])
