 abm_regular <- function(days, delta_t, y, pars, starting = NULL, temp_C_fun = temp_C_fun, temp_air_C_fun = temp_air_C_fun, pH_fun = pH_fun, H2SO4_inhibition_fun = H2SO4_inhibition_fun) { 
  ...
  for (i in 1:n_int) {
    ...
    out <- deSolve::lsoda(y = y, times = times, rates, parms = pars, temp_C_fun = temp_C_fun, temp_air_C_fun = temp_air_C_fun, pH_fun = pH_fun, H2SO4_inhibition_fun = H2SO4_inhibition_fun)

    # Get number of microbial groups
    n_mic <- length(pars$qhat_opt)

    # Extract new state variable vector from last row
    y <- out[nrow(out), 1:(length(c(pars$qhat_opt)) + 15) + 1]
    ...
    if (wash[i]) {
      ...
      if (pars$rest_d > 0) {
        ...
        outr <- deSolve::lsoda(y = y, times = times, rates, parms = parsr, temp_C_fun = temp_C_fun, temp_air_C_fun = temp_air_C_fun, pH_fun = pH_fun, H2SO4_inhibition_fun = H2SO4_inhibition_fun)
        # Extract new state variable vector from last row
        y <- outr[nrow(outr), 1:(length(c(pars$qhat_opt)) + 15) + 1]
