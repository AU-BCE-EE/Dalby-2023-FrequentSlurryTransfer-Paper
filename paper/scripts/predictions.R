# Prediction of storage emissions with different baseline methane emissions: used in part of fig_predict
...
# model parms applying to all sections.
...
summary_storage <-  summarise(group_by(pred_all, treat, i), CH4_emis_rate = mean(CH4_emis_rate, na.rm = T)) %>%
  ungroup()  %>%  mutate(alpha_opt = rep(pars_test$alpha_opt, 4), qhat_opt = rep(pars_test$qhat_opt, 4)) %>% 
  pivot_wider(names_from = treat, values_from = CH4_emis_rate) %>% select(-i)
